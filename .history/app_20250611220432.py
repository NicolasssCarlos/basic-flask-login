...
connection = sqlite3.connect("users_log.db", check_same_thread=False)
...
def signup():
    ...
    if request.method == "POST":
        ...
        if name and username and email and password:
            ...
            if not errors:
                cursor.execute(
                    "INSERT INTO users_log (name, username, email, password) VALUES (?, ?, ?, ?)",
                    (name, username, email, password)
                )
                connection.commit() # Added commit to save changes to the database
